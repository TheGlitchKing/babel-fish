#!/usr/bin/env python3
"""
grader.py — Codebase Mapper Quality Grader
...
def build_report(
    ...
    lines = [
        f"# Codebase Mapper — Installation Report",
        f"",
        f"**Generated**: {now}",
...
def print_terminal_summary(results: list[GradeResult], total_score: float, passed: bool,
    ...
    print(f"\n{CYAN}{'─' * 55}{RESET}")
    print(f"{BOLD}  Codebase Mapper — Grade Report  "
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Grade codebase mapper output')
